...
def extract_train(folder: str, file: str, zip_name: str, labels: dict) -> None:

    with open(file, mode='r') as infile:
        reader = csv.reader(infile)
        next(reader)
        mapper = {rows[1]: int(rows[0]) for rows in reader}

    with ZipFile(zip_name) as zip:
        sub_folder = re.search('data\/(\w+.*)\.zip', zip_name).group(1)
        re_exp = '{}\/(\w+.*)\.png'.format(sub_folder)

        for name in tqdm(zip.namelist()):
            try:
                file_name = re.search(re_exp, name).group(1)
                label_name = labels[mapper[file_name]]

                source = zip.open('{}/{}.png'.format(sub_folder, file_name))
                target = open(os.path.join(folder + '/' + label_name, file_name + '.png'), "wb")
...
def extract_test(folder: str, zip_name: str) -> None:
    ...
    with ZipFile(zip_name) as zip:
        ...
        for name in tqdm(zip.namelist()):
            ...
            try:
                ...
                source = zip.open('{}/{}.png'.format(sub_folder, file_name))
                target = open(os.path.join(folder, file_name + '.png'), "wb")
